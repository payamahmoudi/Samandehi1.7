...
import { sampleSchools, samplePersonnel, sampleSubjectRequirements, sampleBalanceRecords, sampleScheduleEntries, sampleReferenceData } from '../data/sampleData';
...
export function useStore(workspaceId = 'admin-paya') {
  ...
  const [scheduleEntries, setScheduleEntries] = useState<ScheduleEntry[]>(
    stored?.scheduleEntries || sampleScheduleEntries
  );
  const [referenceData, setReferenceData] = useState<ReferenceData | null>(
    stored?.referenceData || sampleReferenceData
  );
    ...
    import('../services/api').then(({ apiGetWorkspace }) => {
      apiGetWorkspace().then((res: any) => {
        if (res?.data) {
          ...
          if (d.referenceData) setReferenceData(d.referenceData);
          // Also cache locally
  ...
  useEffect(() => {
    ...
    setScheduleEntries(next?.scheduleEntries || sampleScheduleEntries);
    setReferenceData(next?.referenceData || sampleReferenceData);
  }, [storageKey]);

  const save = useCallback((s: School[], p: Personnel[], sr: SubjectRequirement[], br: BalanceRecord[], se: ScheduleEntry[] = scheduleEntries, rd: ReferenceData | null = referenceData) => {
    saveToStorage(storageKey, { schools: s, personnel: p, subjectRequirements: sr, balanceRecords: br, scheduleEntries: se, referenceData: rd });
  }, [storageKey, scheduleEntries, referenceData]);
  ...
  const importData = useCallback((data: { schools?: School[], personnel?: Personnel[], subjectRequirements?: SubjectRequirement[], balanceRecords?: BalanceRecord[], scheduleEntries?: ScheduleEntry[], referenceData?: ReferenceData }) => {
    ...
    const newRD = data.referenceData || referenceData;
    if (data.referenceData) setReferenceData(data.referenceData);
    save(newSchools, newPersonnel, newSR, newBR, newSE, newRD);
  }, [schools, personnel, subjectRequirements, balanceRecords, scheduleEntries, referenceData, save]);
  ...
  const resetData = useCallback(() => {
    ...
    setScheduleEntries(sampleScheduleEntries);
    setReferenceData(sampleReferenceData);
  }, [storageKey]);
