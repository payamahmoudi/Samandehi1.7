...
type TabType = 'dashboard' | 'schools' | 'personnel' | 'assignments' | 'analysis' | 'import' | 'admin' | 'history' | 'reports' | 'schedule' | 'announcements';
...
const DEFAULT_YEARS = ['۱۴۰۱-۱۴۰۲', '۱۴۰۲-۱۴۰۳', '۱۴۰۳-۱۴۰۴', '۱۴۰۴-۱۴۰۵'];
const CURRENT_YEAR = '۱۴۰۴-۱۴۰۵';
...
export default function App() {
  ...
  const [academicYears, setAcademicYears] = useState<string[]>(() => {
    try { const s = localStorage.getItem('saman-academic-years'); return s ? JSON.parse(s) : DEFAULT_YEARS; } catch { return DEFAULT_YEARS; }
  });
  const [academicYear, setAcademicYear] = useState(() => localStorage.getItem('saman-academic-year') || CURRENT_YEAR);
  const [announcements, setAnnouncements] = useState<{id:string;text:string;date:string}[]>(() => {
    try { const s = localStorage.getItem('saman-announcements'); return s ? JSON.parse(s) : []; } catch { return []; }
  });
  const auth = useAuth();
  ...
  const store = useStore(`${auth.currentUser?.id || 'guest'}-${academicYear}`);
    ...
    <div className={`flex min-h-screen w-full ${minBg} transition-colors duration-200`}>
          ...
          <div className="flex items-center gap-3">
            ...
            {sidebarOpen && (
              <div className="animate-fade-in">
                <h1 className="font-bold text-sm leading-tight">سامانه پایا</h1>
      ...
      <main className={`flex-1 min-w-0 transition-all duration-300 ${sidebarOpen ? 'md:mr-64' : 'md:mr-16'} ${auth.impersonating ? 'pt-10' : ''}`}>
          ...
          <div className="flex justify-between items-center gap-2">
            ...
            <div className="flex items-center gap-1 md:gap-3 flex-shrink-0">
              ...
              <select value={academicYear} onChange={e => {
                ...
                if (newYear === academicYear) return;
                if (confirm(`⚠️ تغییر سال تحصیلی به ${newYear}\n\nتوصیه: قبل از تغییر سال، از داده‌های فعلی پشتیبان بگیرید.\n\nآیا ادامه می‌دهید؟`)) {
                  setAcademicYear(newYear);
                  localStorage.setItem('saman-academic-year', newYear);
                }
              }} dir="ltr" className="px-2 py-1 border border-indigo-200 rounded-lg text-[10px] md:text-xs bg-indigo-50 text-indigo-700 font-medium text-center" style={{ direction: 'ltr' }}>
              ...
              <button onClick={() => {
                try {
                  const { exportEncryptedBackup } = require('./utils/crypto');
                  exportEncryptedBackup(auth.currentUser?.id || 'guest', auth.currentUser?.fullName, auth.currentUser?.username);
                } catch { alert('خطا در پشتیبان‌گیری'); }
              }} className="hidden md:block rounded-lg bg-emerald-50 border border-emerald-200 px-2 py-1.5 text-xs font-medium text-emerald-700 hover:bg-emerald-100" title="پشتیبان‌گیری سریع">
                💾
              </button>
              <button
                onClick={() => {
                  const choice = confirm('آیا از خروج مطمئن هستید؟\n\n⚠️ قبل از خروج پشتیبان بگیرید.\n\n«تأیید» = خروج بدون پشتیبان\n«لغو» = ماندن در سامانه');
                  if (choice) auth.logout();
                }}
        ...
        <div className="p-3 md:p-6 animate-fade-in">
          ...
          {activeTab === 'analysis' && (
            <SubjectAnalysis
              ...
              subjectRequirements={store.subjectRequirements}
              balanceRecords={store.balanceRecords}
              referenceData={store.referenceData || undefined}
            />
          )}
          {activeTab === 'import' && (
            <>
          ...
          {activeTab === 'admin' && auth.currentUser.role === 'admin' && (
            <AdminPanel
              ...
              onAddYear={(year: string) => {
                let updated: string[];
                if (year.startsWith('__DELETE__')) {
                  const toDelete = year.replace('__DELETE__', '');
                  updated = academicYears.filter(y => y !== toDelete);
                } else {
                  // Convert to Persian numerals
                  const persianYear = year.replace(/[0-9]/g, (d: string) => '۰۱۲۳۴۵۶۷۸۹'[parseInt(d)]);
                  if (academicYears.includes(persianYear)) return;
                  updated = [...academicYears, persianYear];
                }
                setAcademicYears(updated);
                localStorage.setItem('saman-academic-years', JSON.stringify(updated));
              }}
            />
          )}
          {activeTab === 'reports' && (
            ...
            <div className="space-y-4">
              ...
              {isAdmin && (
                <div className="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-2xl p-6 text-white shadow-lg">
                  <h3 className="text-lg font-bold mb-4">📢 ارسال اطلاعیه جدید</h3>
                  <textarea id="new-announcement" rows={3} placeholder="متن اطلاعیه را وارد کنید..." className="w-full rounded-xl px-4 py-3 text-gray-800 text-sm resize-none" />
                  <button onClick={() => {
                    const el = document.getElementById('new-announcement') as HTMLTextAreaElement;
                    if (!el?.value.trim()) return alert('متن اطلاعیه را وارد کنید.');
                    const newAnn = { id: `ann-${Date.now()}`, text: el.value.trim(), date: new Date().toLocaleDateString('fa-IR') };
                    const updated = [newAnn, ...announcements];
                    setAnnouncements(updated);
                    localStorage.setItem('saman-announcements', JSON.stringify(updated));
                    el.value = '';
                  }} className="mt-3 px-6 py-2 bg-white text-indigo-700 rounded-xl text-sm font-bold hover:bg-indigo-50">
                    ✅ انتشار اطلاعیه
                  </button>
                </div>
              )}
              ...
              {announcements.length === 0 ? (
                <div className="bg-white rounded-xl border border-gray-100 p-12 text-center shadow-sm">
                  <div className="text-5xl mb-4">📭</div>
                  <p className="text-gray-400">اطلاعیه‌ای ثبت نشده است.</p>
                </div>
              ) : (
                <div className="space-y-3">
                  {announcements.map(ann => (
                    <div key={ann.id} className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
                      <div className="flex justify-between items-start">
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-gray-700 leading-relaxed whitespace-pre-wrap">{ann.text}</p>
                        </div>
                        {isAdmin && (
                          <button onClick={() => {
                            ...
                            setAnnouncements(updated);
                            localStorage.setItem('saman-announcements', JSON.stringify(updated));
                          }} className="text-red-400 hover:text-red-600 text-sm mr-2">🗑️</button>
                        )}
                      </div>
                    </div>
                  ))}
        ...
        <footer className="border-t border-gray-100 bg-white px-3 md:px-6 py-3 text-center text-[10px] md:text-xs text-gray-400">
          سامانه پایا - ساماندهی نیروی انسانی | طراح: پایا محمودی
        </footer>
      </main>
