...
export default function AdminPanel({ users, currentUser, onAddUser, onUpdateUser, onRemoveUser, onImpersonate, academicYears, onAddYear }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
        <h3 className="text-sm font-semibold text-gray-700 mb-4">📅 مدیریت سال‌های تحصیلی</h3>
        <div className="flex flex-wrap gap-2 mb-4">
          {academicYears.map(y => (
            <span key={y} className="inline-flex items-center gap-1 px-3 py-1.5 bg-indigo-50 text-indigo-700 rounded-lg text-xs font-medium border border-indigo-200" dir="ltr">
              {y}
              <button onClick={() => {
                if (!confirm(`حذف سال ${y}؟`)) return;
                onAddYear('__DELETE__' + y);
                setMessage(`سال ${y} حذف شد.`);
              }} className="text-red-400 hover:text-red-600 mr-1">✕</button>
            </span>
          ))}
        </div>
        <div className="flex gap-2 items-end">
          <div className="flex-1">
            <label className="text-[10px] text-gray-500 mb-1 block">افزودن سال جدید</label>
            <input value={newYear} onChange={e => setNewYear(e.target.value)} placeholder="مثلاً ۱۴۰۶-۱۴۰۵" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" />
          </div>
          <button onClick={() => {
            if (!newYear.trim()) return;
            if (academicYears.includes(newYear.trim())) { setMessage('این سال قبلاً اضافه شده.'); return; }
            onAddYear(newYear.trim());
            setNewYear('');
            setMessage('✅ سال جدید اضافه شد.');
