...
import { calculateTrazBalance, generateSchoolLessonMaps, calculateWorkforceAnalysis, TrazResult, SchoolLessonMap, WorkforceAnalysisRow, ReferenceData } from '../utils/excelImport';
...
export default function SubjectAnalysis({ schools, personnel, subjectRequirements, balanceRecords, referenceData }: Props) {
  ...
  // Workforce Analysis (Greedy allocation)
  const workforceAnalysis: WorkforceAnalysisRow[] = useMemo(() =>
    (balanceRecords && referenceData && referenceData.qualifications.length > 0)
      ? calculateWorkforceAnalysis(balanceRecords, personnel, referenceData) : [],
  [balanceRecords, personnel, referenceData]);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Workforce Analysis - Greedy Allocation */}
      {workforceAnalysis.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
          <h3 className="text-sm font-semibold text-gray-700 mb-4">🧮 تحلیل نیرو (بدون شمارش مضاعف)</h3>
          <div className="grid grid-cols-3 gap-3 mb-4">
            <div className="bg-red-50 rounded-lg p-3 text-center border border-red-200">
              <div className="text-2xl font-bold text-red-700">{workforceAnalysis.filter(w => w.status === 'کمبود').length}</div>
              <div className="text-xs text-red-600">گروه دارای کمبود</div>
            </div>
            <div className="bg-green-50 rounded-lg p-3 text-center border border-green-200">
              <div className="text-2xl font-bold text-green-700">{workforceAnalysis.filter(w => w.status === 'متوازن').length}</div>
              <div className="text-xs text-green-600">گروه متوازن</div>
            </div>
            <div className="bg-blue-50 rounded-lg p-3 text-center border border-blue-200">
              <div className="text-2xl font-bold text-blue-700">{workforceAnalysis.filter(w => w.status === 'مازاد').length}</div>
              <div className="text-xs text-blue-600">گروه دارای مازاد</div>
            </div>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="py-2 px-2 text-right">گروه تدریس</th>
                  <th className="py-2 px-2 text-center">نیاز (ساعت)</th>
                  <th className="py-2 px-2 text-center">ظرفیت (ساعت)</th>
                  <th className="py-2 px-2 text-center">نفرات</th>
                  <th className="py-2 px-2 text-center">تراز</th>
                  <th className="py-2 px-2 text-center">≈ نفر</th>
                  <th className="py-2 px-2 text-center">وضعیت</th>
                </tr>
              </thead>
              <tbody>
                {workforceAnalysis.map(w => (
                  <tr key={w.groupCode} className={`border-t border-gray-50 ${w.status === 'کمبود' ? 'bg-red-50/30' : w.status === 'مازاد' ? 'bg-blue-50/30' : ''}`}>
                    <td className="py-1.5 px-2 font-mono">{w.groupCode}</td>
                    <td className="py-1.5 px-2 font-medium">{w.groupName}</td>
                    <td className="py-1.5 px-2 text-center">{w.requiredHours}</td>
                    <td className="py-1.5 px-2 text-center">{w.capacityHours}</td>
                    <td className="py-1.5 px-2 text-center">{w.personnelCount}</td>
                    <td className="py-1.5 px-2 text-center">
                      <span className={`font-bold ${w.balance < 0 ? 'text-red-600' : w.balance > 0 ? 'text-blue-600' : 'text-green-600'}`}>{w.balance}</span>
                    </td>
                    <td className="py-1.5 px-2 text-center font-medium">{w.equivalentPersons}</td>
                    <td className="py-1.5 px-2 text-center">
                      <span className={`px-2 py-0.5 rounded text-[10px] ${w.status === 'کمبود' ? 'bg-red-100 text-red-700' : w.status === 'مازاد' ? 'bg-blue-100 text-blue-700' : 'bg-green-100 text-green-700'}`}>{w.status}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
